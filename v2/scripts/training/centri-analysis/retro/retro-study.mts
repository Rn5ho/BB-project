// Retro-prescription study: put the smart-pass prototype at each Centri player's FIRST
// observed state, sweep the owner-knowledge builds, prescribe the best plan — then
// simulate the plan AND the club's actual logged training from the same start, and
// compare both against the actually-observed end state.
//
// Usage (from v2/): npx tsx scripts/training/centri-analysis/retro/retro-study.mts <outDir> [--player "Name"]
...
function staffAt(author: string, date: string) {
  if (author === 'pjtr576') {
    return {
...
for (const name of players) {
  ...
  for (const w of clubWeeks) {
    ...
    const staff = staffAt(author, w.report_date);
