...
export default function NotesSection({ playerId, notes, readOnly = false }: { playerId: number; notes: { id: number; body: string; createdAt: string }[]; readOnly?: boolean }) {
      ...
      <ul className="space-y-2">
        {notes.map((n) => (
          <li key={n.id} className="flex items-start gap-2 text-sm border border-neutral-800 rounded px-3 py-2">
            ...
            {!readOnly && <button disabled={pending} onClick={() => start(async () => { await deleteNote(playerId, n.id); })} className="text-neutral-600 hover:text-red-400">×</button>}
          </li>
        ))}
