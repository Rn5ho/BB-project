...
- **Daily cron moved to Hetzner**: `/home/btcedge/bb-scout/bb-daily-cron.sh` in btcedge's crontab (06:00 UTC) runs the market sweep locally (`MARKET_MAX_PAGES=100 npx tsx scripts/market-sweep.mts` — two-pass: newest-first + oldest-first flood recovery when BB's 1000-result window is exhausted), then curls the Vercel `/api/cron/daily?skip=market` endpoint with CRON_SECRET read from the env file for seasons/minutes/inference/players. Market moved off Vercel 2026-08-03: the sweep inside the route regularly blew the 300 s `maxDuration` (504s on Jul 20/27, Aug 3), and Vercel could only ever see the newest 900 of a season-end listing flood. Vercel cron removed from `vercel.json` — Hetzner is now the single reliable scheduler.
