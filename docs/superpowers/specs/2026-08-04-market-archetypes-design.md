...
- **Anchor — three U-21 milestones (owner-specified 2026-08-04, supersedes the two-checkpoint wording)**: **M1** = entering age 21 week 1: PLAYABLE (squad selection gate — full targets minus finishing capacity: floor −2, definers −1). **M2** = entering age-21 season week ~7 (`FINALIZE_WEEK`): FINALIZED — group stage ends and playoffs begin (Euro/Worlds/consolation all rhyme), full targets due here. **After M2**: polish only — elite-pushing touches that never make-or-break selection. Implemented as a staged beam search (phase 1 to M1 playable targets, phase 2 across the 6 group-stage weeks to full targets, phase 3 extension); feasibility reports both verdicts. Greek observed closure capacity ≤ +2 OD / +3 ID per half-season backs the −2/−1 playable deltas. End-of-21 reported as stretch line only. Compute horizons with `horizonWeeks(now, {age, week})` — never `board.weeksToEndOfAge21` (stale convention; fix or bypass).
