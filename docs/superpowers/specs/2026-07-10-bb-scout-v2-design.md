...
1. **Players JSON API — no auth required.** `GET https://api.buzzerbeater.com/BBAPI/api/Players?countryId=66&minAge=18&maxAge=21` returns all 805 Slovenian 18–21yo: `playerId`, name, `teamId`/`teamName` (owner), `position`, `age`, `potential`, `salary`, `height`, `gs` (game shape), `dmi`, `isForSale`, `isInjured`, `seasonDrafted`, `isUtopian`. Works for any country. Caps at 1000 rows (`isMoreThan1000` flag) — filter params keep results under the cap. This is the same API nt.buzzerbeater.com (community scout site) uses; building on it makes that site redundant for our workflow.
